...
const myArr5 = data;
while(keepGoin3){
const myArr4  = myOpt.match(inside);
myArr5 = myArr5[myArr4[0]];
myOpt.replace(("\/" + myArr4[0]), '');
let keepGoin3 = myOpt.match(inside) != null;
}
myArr5 = myArr5[myOpt]
myArr6 = await myArray6Thing(myArr5);
  async function myArray6Thing(array_input){
    newArray = [];
    for(let i of array_input){
    if(Array.isArray(i){
      await newArray = newArray.concat(myArray6Thing(newArray));
    else{
